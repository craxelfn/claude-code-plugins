...
LIVE_TEST_ROWS = [
    ...
    LiveTestRow(4,  "aidp-atp",            "Wallet (mTLS)",                      "atp_wallet_query.ipynb",         "Non-zero rows from a known ATP schema",                            True),
    LiveTestRow(5,  "aidp-atp",            "IAM DB-Token (>25 min)",             "atp_dbtoken_refresh.ipynb",      "On-executor token refresh, zero auth failures",                    True),
    LiveTestRow(6,  "aidp-atp",            "API Key + inline OCI config",        "atp_admin_apikey.ipynb",         "Admin-only DDL or DBA_* query succeeds",                           False),
    LiveTestRow(7,  "aidp-exacs",          "Wallet (TCPS)",                      "exacs_wallet_query.ipynb",       "Non-zero rows over TCPS port 1522",                                False),
    LiveTestRow(8,  "aidp-exacs",          "IAM DB-Token",                       "exacs_dbtoken_query.ipynb",      "Same query via DB-token; skip if cluster not IAM-enabled",         False),
    LiveTestRow(9,  "aidp-exacs",          "Legacy DB user/password",            "exacs_user_password.ipynb",      "Same query via classic credentials",                               False),
    LiveTestRow(10, "aidp-bds-hive",       "Kerberos keytab",                    "bds_hive_kerberos.ipynb",        "kinit succeeds, Hive JDBC SHOW TABLES returns rows",               False),
    LiveTestRow(11, "aidp-bds-hive",       "LDAP",                               "bds_hive_ldap.ipynb",            "Same query via LDAP bind",                                         False),
    LiveTestRow(12, "aidp-fusion-rest",    "HTTP Basic",                         "fusion_rest_basic.ipynb",        "<=499-row paged fetch lands as Spark DataFrame",                   True),
    LiveTestRow(13, "aidp-fusion-rest",    "OAuth (Fusion OAuth profile)",       "fusion_rest_oauth.ipynb",        "Same fetch via bearer token; auto-refresh on 401",                 False),
    LiveTestRow(14, "aidp-fusion-bicc",    "HTTP Basic + API Key",               "fusion_bicc_to_dataframe.ipynb", "Extract kicks off -> CSV in OS bucket -> Spark reads it",          True),
    LiveTestRow(15, "aidp-epm-cloud",      "Identity-domain OAuth",              "epm_planning_oauth.ipynb",       "Planning REST applications=200; MDX export -> DataFrame",          False),
    LiveTestRow(16, "aidp-epm-cloud",      "Basic (legacy tenancy.user@domain)", "epm_planning_basic.ipynb",       "Same MDX export via Basic",                                        False),
    LiveTestRow(17, "aidp-essbase",        "HTTP Basic",                         "essbase_mdx_basic.ipynb",        "MDX SELECT returns DataFrame with expected dim count",             False),
    LiveTestRow(18, "aidp-streaming-kafka", "SASL_SSL OAuth",                    "kafka_streaming_oauth.ipynb",    "query.lastProgress shows non-zero numInputRows",                   False),
    LiveTestRow(19, "aidp-streaming-kafka", "SASL/PLAIN with API Key",           "kafka_streaming_apikey.ipynb",   "Same stream via API-key SASL",                                     False),
]
