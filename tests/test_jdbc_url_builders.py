...
def test_hive_url_kerberos_default_port():
    url = build_hive_jdbc_url(
        hs2_host="hs2.example.com",
        database="default",
        hs2_principal="hive/hs2.example.com@EXAMPLE.COM",
    )
    assert url == (
        "jdbc:hive2://hs2.example.com:10000/default"
        ";principal=hive/hs2.example.com@EXAMPLE.COM"
        ";auth=kerberos"
    )


def test_hive_url_kerberos_custom_port_and_db():
    url = build_hive_jdbc_url(
        hs2_host="nitishun0-0.rgroverprdpub1.rgroverprd.oraclevcn.com",
        hs2_port=10010,
        database="test",
        hs2_principal="hive/nitishun0-0.rgroverprdpub1.rgroverprd.oraclevcn.com@NITISH.ORACLE.COM",
    )
    assert url == (
        "jdbc:hive2://nitishun0-0.rgroverprdpub1.rgroverprd.oraclevcn.com:10010/test"
        ";principal=hive/nitishun0-0.rgroverprdpub1.rgroverprd.oraclevcn.com@NITISH.ORACLE.COM"
        ";auth=kerberos"
    )


def test_hive_url_no_principal_no_auth():
    url = build_hive_jdbc_url(hs2_host="hs2.example.com", database="db1")
    assert url == "jdbc:hive2://hs2.example.com:10000/db1"


def test_hive_url_http_transport():
    url = build_hive_jdbc_url(
        hs2_host="hs2.example.com",
        database="default",
        hs2_principal="hive/hs2.example.com@EXAMPLE.COM",
    ...
    assert ";transportMode=http" in url
    assert ";auth=kerberos" in url


def test_hive_url_extra_props():
    url = build_hive_jdbc_url(
        hs2_host="hs2.example.com",
        database="default",
        extra_props={"hive.server2.thrift.sasl.qop": "auth-conf"},
    )
    assert url.endswith(";hive.server2.thrift.sasl.qop=auth-conf")
