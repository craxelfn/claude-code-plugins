"""Unit tests for ``orchestrator/state_phase2.py`` (Phase 2 Step 10)."""
...
# ---------------------------------------------------------------------------
# PHASE2_NEW_COLUMNS — locks the migration column set
# ---------------------------------------------------------------------------


class TestPhase2NewColumns:
    def test_includes_pack_profile_identity(self) -> None:
        names = {n for n, _ in PHASE2_NEW_COLUMNS}
...
class TestEnsureStateColumnsV2:
    ...
    def test_migration_emits_alter_only_for_missing_columns(self, monkeypatch) -> None:
        """If 3 of the 17 Phase 2 columns are already present, the ALTER
        ...
        # aren't in PHASE2_NEW_COLUMNS so they're orthogonal).
    ...
    def test_migration_is_noop_when_all_phase2_columns_present(self, monkeypatch) -> None:
        """If every Phase 2 column already exists, no ALTER is emitted —
        ...
        monkeypatch.setattr(
            v1_state, "_existing_state_columns",
            lambda spark, table: {n for n, _ in PHASE2_NEW_COLUMNS},
        )
        build_calls = []
        monkeypatch.setattr(
    ...
    def test_migration_idempotency_second_call_is_noop(self, monkeypatch) -> None:
        ...
        # Simulate the table starting empty of Phase 2 cols, then having
        ...
        def fake_existing(spark, table):
            ...
            if call_state["count"] == 1:
                return set()
            return {n for n, _ in PHASE2_NEW_COLUMNS}
        ...
        assert len(alter_calls[0]) == len(PHASE2_NEW_COLUMNS)


# ---------------------------------------------------------------------------
# Latest view DDL — Phase 2 grain locked
# ---------------------------------------------------------------------------


class TestPhase2LatestViewDDL:
    ...
    def test_view_projects_phase2_columns(self) -> None:
        ddl = _phase2_latest_view_ddl("t", "v")
