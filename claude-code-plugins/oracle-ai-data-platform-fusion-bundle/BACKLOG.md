...
### `[x]` P1.5ε-fix1 — BICC credential preflight (endpoint shape probe + check 6 wiring) — **shipped 2026-06-03** in bundle `p1.5e-postship-dispatch-ux`
**Done**: Step 0 endpoint probed live against `playground` → `GET /aiDataPlatforms/<aidp-id>/credentials` (data-lake scope, NOT workspace-scoped; single-resource GET takes UUIDs only). `AidpRestClient.check_credential_exists(name)` LISTs + walks `items[]`. Wired as Phase B check 6 in `dispatch/preflight.py` — **always checks** regardless of `bundle.fusion.password` shape per reviewer correction (the cluster-side creds-cell unconditionally fetches the AIDP secret). 6 unit tests + 1 live TC29 Probe 6. Acceptance criterion `<2s` smashed: live missing-credential FAIL in **4.77s end-to-end** vs ~4 min mid-notebook failure pre-fix. Full shape captured in `dev/RESEARCH_aidp_rest_api_probe_results.md` §11.
