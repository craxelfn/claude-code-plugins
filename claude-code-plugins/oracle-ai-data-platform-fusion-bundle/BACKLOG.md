...
### `[x]` P1.5ε-fix1 — BICC credential preflight (endpoint shape probe + check 5 wiring) — **shipped 2026-06-03** in bundle `p1.5e-postship-dispatch-ux`
**Done**: Step 0 endpoint probed live against `playground` → `GET /aiDataPlatforms/<aidp-id>/credentials` (data-lake scope, NOT workspace-scoped; single-resource GET takes UUIDs only). `AidpRestClient.check_credential_exists(name)` LISTs + walks `items[]`. Wired as Phase B **check 5 — sits AHEAD of the cluster-state check 6** (reviewer-driven correction landed in commit `f577862`; an earlier draft put it after cluster-state, which would have spent ~5min auto-starting a STOPPED cluster before fast-failing on missing credential, negating the entire fast-fail invariant). Cluster-state SKIPs when credential FAILs — no `start_cluster` / `wait_cluster_active` invoked. **Always checks** regardless of `bundle.fusion.password` shape per reviewer correction round 1 (the cluster-side creds-cell unconditionally fetches the AIDP secret). Remediation hint references the configured `biccSecretKey` (NOT a hardcoded `'password'`) per reviewer round 3. **8 unit tests** + **3 live TC29 sub-probes** (6a happy path, 6b missing-credential fast-fail, 6c custom-key remediation). Acceptance criterion `<2s for the check itself` smashed: live missing-credential FAIL in **2.53s end-to-end wall** (was 4.77s in pre-reorder draft of the evidence); with a STOPPED cluster the wall savings vs the original ordering is ~5min. Full endpoint shape captured in `dev/RESEARCH_aidp_rest_api_probe_results.md` §11.

**Original entry preserved below for context.**

**Why**: P1.5ε ships 5 preflight checks instead of the planned 6 — the BICC credential-store presence check (`GET /credentials/<name>`) was dropped because the AIDP credential REST endpoint shape was not empirically confirmed against `fusion_bundle_dev` before P1.5ε implementation. Residual operator pain: a missing `fusion_bicc_password` entry surfaces mid-notebook (~4 min into dispatch, after wheel build + upload + job submit + cluster spin-up + cluster-side creds-cell execution) instead of at preflight (~300ms — same shape as the other Phase B checks). Acceptance criterion explicit in the original idea.md was "exits 2 in <2s with `BICC credential '<name>' not found`".
**When to revisit**: any of (a) Oracle publishes the credential-store endpoint shape in the `aiwap` doc tree, (b) an operator reports a real onboarding failure traceable to a missing-credential mid-notebook surprise, (c) a tenant probe surfaces the actual REST contract during unrelated work.
**Fix sketch**: probe via `oci raw-request` against `playground` (per the original Step 0 decision matrix — `oci raw-request --target-uri ".../workspaces/<wk>/credentials"` then ".../credentials/<name>"). Capture the response shape in `dev/RESEARCH_aidp_rest_api_probe_results.md` §11. Add `AidpRestClient.check_credential_exists(name) → bool` matching the actual shape. Wire as Phase B check 6 in `dispatch/preflight.py:run_remote_preflight` after the cluster-state check (cheaper than start-and-wait so it can move ahead of `_check_cluster_state` if useful). Drop the explicit "5 checks" docstring note in `preflight.py`.
**Size**: S — ~30 min endpoint probe + ~1h client primitive + ~1h preflight wiring + unit tests + TC29 probe-6 capture.
**Depends on**: nothing technically (live tenant access at probe time, but no plugin code blocker).
**Accept**: preflight reports `BICC credential '<name>' not found in AIDP credential store for workspace <key> — add it via the AIDP UI before running` in <2s when the named entry is absent; PASS in <300ms when present; new unit tests cover both shapes; TC29 evidence file gains probe 6 covering the missing-credential fast-fail.
**Cross-ref**: P1.5ε §Step 5 (preflight check 6 dropped); `tests/live/TC29_rest_dispatch.md` (residual risk acknowledged); `docs/rest_dispatch_setup.md` §4 (operator must hand-verify the entry exists until this fix ships).

### `[x]` P1.5ε-fix5 — Resume via REST dispatch (`--resume <run_id>` over the REST path) — **shipped 2026-06-03** in PR #19 on `P1.5ε-fix5`
**Done**: `resume_run_id` threaded through `dispatch_via_rest` → `build_notebook` → `_build_run_cell` as a `repr()`-quoted literal (same pattern as fix9's mode/datasets/layers). CLI guard at `commands/run.py:114-124` lifted; symmetric "Resuming run <id>" cyan banner gated on `resume_run_id is not None and not dry_run` (suppressed under `--dry-run` so the operator doesn't see a misleading banner before the short-circuit). `--resume` help text scrubbed of `--inline` / fix5 references; now reads as a stable feature on both paths.

Bonus hardening surfaced by the reviewer + TC29b live evidence:
- **Marker-parse regex fallback** for the TC27 trap (AIDP's `display_data text/plain` strips JSON-escape backslashes from failed-step `error_message=repr(exc)`). New typed exception `DispatchMarkerDegradedError(code="DISPATCH_MARKER_DEGRADED")` carries the recovered `run_id` in its message so `--resume <id>` is copy-pastable from the CLI's red error block. Without this, the typical mid-cascade failure shape produces an unrecoverable marker → TC29b's Phase 2 → Phase 3 transition isn't reachable.
- **Cell-error enrichment** on non-SUCCESS terminal status — extends fix8's `extract_cell_errors` so a bad `--resume <id>` surfaces the typed `ResumeRunNotFoundError` / `ResumeRunNotResumableError` / `ResumeBundleMismatchError` ename in the operator's error block without notebook archaeology. TC29b Phase 4 forced a live discovery: AIDP emits cell exceptions as `output_type=stream, name=stderr` with the Python traceback inline, NOT as the documented `output_type=error`. `extract_cell_errors` extended with a single-line regex on stderr streams to recognize both shapes; chained tracebacks correctly surface the outermost exception.

**Live evidence** (`tests/live/TC29b_resume_via_rest.md`): four-phase capture against `fusion_bundle_dev`. Phase 1 (clean baseline, 5.6 min) → Phase 2 (induced dim_supplier silver failure, 3.9 min, **regex-fallback branch fired** — JSON parse hit "Expecting ',' delimiter at column 4427" exactly where `repr(RuntimeError("..."))` produced unescaped nested quotes; recovered `run_id=66aa575a…` from regex fallback) → Phase 3 (`--resume <phase-2-run-id>`, 2.4 min, SUCCESS with 3× `resumed_skipped` carry-forwards + 2× successful re-attempts under the **original** `run_id`; SOX-trail `silver_run_id matches on 209/209 rows`, `gold_run_id matches on 309/309 rows`) → Phase 4 (bad `--resume tc29b-not-a-real-id`, 43.6 s, FAILED at cell 3 with the literal `state.py:954-955` message). Resume vs clean wall = **0.44×** (146.6s / 334.7s). 1013 unit tests green including 6 new `TestExtractCellErrorsStderrStream` cases.

**Original entry preserved below for context.**

### `[~]` P1.5ε-fix5 — Resume via REST dispatch (`--resume <run_id>` over the REST path)
**Why**: `--resume <run_id>` is `--inline`-only today; the dispatch path's CLI guard at `commands/run.py:run()` rejects the combo with `OrchestratorConfigError("--resume requires --inline; REST-dispatch resume is tracked as BACKLOG P1.5ε-fix5")`. Hands-off scheduled refresh + CI regression probes both want resume-on-failure semantics over REST without spinning up a notebook session.
**When to revisit**: any of (a) an operator hits a partial-failure on a long-running REST dispatch that's expensive to redo from scratch, (b) scheduled-refresh / cron orchestration ships (today operators use AIDP scheduler), (c) CI starts running orchestrator regressions and wants the resume contract for flake recovery.
**Fix sketch**: thread `resume_run_id: str | None` through `dispatch_via_rest(...)` and `dispatch/notebook_builder.py:build_notebook(...)`. The run-cell currently hardcodes `resume_run_id=None`; lift that to inject the operator's value via `repr()` (same pattern as `mode`/`datasets`/`layers`). Lift the CLI guard at `commands/run.py:run()` so `--resume` + REST is accepted; update `tests/unit/test_commands.py::test_resume_without_inline_exits_2_with_fix5_hint` to match the new behavior. Capture three-phase live evidence at `tests/live/TC29b_resume_via_rest.md` modeled on the existing `.claude/skills/fusion-tc26-run/tc27_dispatch.py` pattern (clean baseline → induced failure → resume → run completes under the original `run_id`).
**Size**: M — ~30 LOC dispatch wiring + ~30 LOC builder + 1 CLI-test update + ~6 new dispatch unit tests + TC29b live capture (~30 min cluster time).
**Depends on**: nothing in plugin code; orchestrator-side resume already handles `resume_run_id` through `--inline`.
**Accept**: `aidp-fusion-bundle run --mode seed --env dev --resume <run_id>` (no `--inline`) succeeds end-to-end against `fusion_bundle_dev`; the resumed run carries the original `run_id` through `silver_run_id`/`gold_run_id` audit columns; TC29b captures three phases (clean → induce failure mid-cascade → resume → cascade completes); unit-tested by patching the notebook builder to assert `resume_run_id=<value>` is injected into the run-cell source.
**Cross-ref**: P1.5ε plan §3.1 + §Step 4 (resume scope boundary); `.claude/skills/fusion-tc26-run/tc27_dispatch.py` (template for the three-phase live probe).

### `[ ]` P1.5ε-fix6 — Cloud-side dispatch signers (vault / resource-principal / instance-principal)
**Why**: P1.5ε ships profile-based OCI signing only — both API-key (classic `oci.signer.Signer`) and session-token (`oci.auth.signers.SecurityTokenSigner`). The existing `AuthSpec.mode: vault` field on `EnvSpec` is **rejected at preflight** with a `fix6` hint; any other mode silently falls back to the wrong identity. That closes off three legitimate dispatch contexts: (a) dispatching from inside OCI Functions (needs resource-principal signer — no `~/.oci/config` available), (b) dispatching from OCI Compute / Container Instances (instance-principal), (c) CI runners using vault-stored API keys instead of file-system-resident profiles.
**When to revisit**: any of (a) a customer asks to dispatch from a Function / Compute (P3 scheduled-refresh patterns), (b) a security review forbids file-system OCI profiles in CI, (c) the vault-mode placeholder in `examples/aidp.config.example.yaml`'s prior `prod` block needs to ship for real.
**Fix sketch**: extend `dispatch/rest_client.py:_build_signer(cfg)` to handle three new modes inferred from `EnvSpec.auth`:
- `mode: vault` → `oci.auth.signers.InstancePrincipalsSecurityTokenSigner` after resolving the API-key fields from the operator's vault OCIDs (`auth.api_key_ocid` + `auth.private_key_ocid`).
- `mode: resource-principal` → `oci.auth.signers.get_resource_principals_signer()` — no key material needed; resolved from `OCI_RESOURCE_PRINCIPAL_*` env vars OCI Functions injects.
- `mode: instance-principal` → `oci.auth.signers.InstancePrincipalsSecurityTokenSigner()` — same shape, picked up from instance metadata service.
Drop the Phase-A coords-check rejection of `mode: vault` (preflight gains per-mode validation instead). Update `docs/rest_dispatch_setup.md` with sections per signer. Tests use `unittest.mock` to patch each signer constructor (no real cloud calls).
**Size**: M — ~80 LOC signer-factory branch + ~30 LOC preflight per-mode validation + ~10 new unit tests + 2 doc sections.
**Depends on**: nothing in plugin code. Cloud-side live validation needs an actual Function / Compute deployment which is out of scope until P3 scheduling.
**Accept**: each of the three new modes produces the right signer instance (tested by mocked constructor identity); preflight accepts `mode: vault` / `mode: resource-principal` / `mode: instance-principal` cleanly; `docs/rest_dispatch_setup.md` documents each scenario; `examples/aidp.config.example.yaml`'s `prod` block can be flipped back to a vault-signer example.
**Cross-ref**: P1.5ε plan §4.4 "Auth scope" (defers vault to fix6); `dispatch/preflight.py:_check_dispatch_coords` (vault-mode rejection point to remove).

### `[x]` P1.5ε-fix7 — `poll_run` timeout as a CLI flag + longer default — **shipped 2026-06-03** in bundle `p1.5e-postship-dispatch-ux`
**Done**: `--poll-timeout` Click flag added to `cli.py:run`; `IntRange(60, 14400)`, default `3600` (bumped from `1800` per TC29 evidence). Threaded through `commands/run.py:run() → _run_via_aidp_dispatch() → dispatch_via_rest(poll_timeout_s=...) → client.poll_run(timeout_s=...)`. 5 unit tests including the BACKLOG-required help-text assertion (`test_poll_timeout_help_text_mentions_default_and_slow_tenant`).

**Original entry preserved below for context.**

**Why**: TC29 probe 4 surfaced this — the dispatcher's default `poll_run(..., timeout_s=1800)` (30 min) is insufficient for slow tenants on a cold-cache BICC extract. Live `erp_suppliers` extract on `saasfademo1` ran past 30 min and triggered `DISPATCH_TIMEOUT` on the laptop side while the cluster job was still legitimately RUNNING. Operators on slow tenants currently have no in-band knob to bump the deadline — they have to hand-edit `dispatch/__init__.py`.
**When to revisit**: ship alongside the first operator report of a `DISPATCH_TIMEOUT` on a real (non-test) run, OR proactively if scheduled-refresh / cron orchestration ships (longer wall-time tolerance becomes the norm).
**Fix sketch**: add `--poll-timeout <seconds>` to `cli.py:run` (Click `IntRange(min=60, max=14400)`; default `3600` — bump from current `1800`). Thread through `commands/run.py:run()` → `_run_via_aidp_dispatch()` → `dispatch_via_rest(..., poll_timeout_s=...)`. `dispatch_via_rest` forwards to `client.poll_run(..., timeout_s=poll_timeout_s)`. Same plumbing pattern as the existing `--datasets` flag.
**Size**: S — ~20 LOC across cli.py + commands/run.py + dispatch/__init__.py + ~3 new tests (default value, override, out-of-range rejection).
**Depends on**: nothing.
**Accept**: `aidp-fusion-bundle run --mode seed --env dev --poll-timeout 7200` accepts and propagates the value; default `--poll-timeout` is 3600 (1h) — verified by the test that captures the default; `--poll-timeout 30` rejected at Click parse (below `min`); operator-facing help text mentions the default + the legitimate slow-tenant use case.
**Cross-ref**: `tests/live/TC29_rest_dispatch.md` Probe 4 (surfaced the limitation); `dispatch/rest_client.py:poll_run` (the underlying primitive).

### `[x]` P1.5ε-fix8 — Partial-progress diagnose-on-timeout for `DispatchPollTimeoutError` — **shipped 2026-06-03** in bundle `p1.5e-postship-dispatch-ux`
**Done**: Added `timeout=` kwargs on `AidpRestClient.get_run` + `fetch_output` (reviewer-driven — `time.monotonic()` budgets alone can't bound a blocking HTTP call). New `_diagnose_partial_progress()` + `_format_cell_progress()` helpers in `dispatch/__init__.py`. On `DispatchPollTimeoutError`, opportunistic `_DIAG_BUDGET_S=10s` enrichment fetches partial executed-notebook + emits per-cell summary; best-effort wrap swallows any diagnostic failure so the original `DISPATCH_TIMEOUT` always surfaces clean. 8 unit tests total: 3 in `test_rest_client.py` (timeout kwarg forwarding), 5 in `test_dispatch_via_rest.py` (enrichment present, **diagnostic calls bounded by `_DIAG_BUDGET_S`** as the reviewer-driven invariant lock, get_run / fetch_output failure paths, no-partial-output clean).

**Original entry preserved below for context.**

**Why**: When `poll_run` deadline fires, today the operator sees `[DISPATCH_TIMEOUT] poll_run(<key>): deadline exceeded after 1800s` and nothing else. To find out where the cluster job is stuck they have to: (a) drop into a Python REPL or `oci raw-request` to fetch the executed notebook, (b) walk `ipynb.metadata.execution_info.current_cell` to see which cell is in flight, (c) inspect cell outputs for partial prints. TC29 Probe 4's diagnostic walk through these steps revealed `pip rc=0` + `FUSION_BICC_PASSWORD loaded` + cell 3 in flight — that should be available out-of-the-box in the timeout message.
**When to revisit**: ship after `P1.5ε-fix7` (or alongside — they're related: better timeout default + better diagnostics when the timeout still fires).
**Fix sketch**: on `DispatchPollTimeoutError` in `dispatch_via_rest`, before re-raising: opportunistically `client.get_run(...)` → resolve `taskRunKey` → `client.fetch_output(taskRunKey)` → walk `ipynb.cells[*]` and print one line per executed cell (cell index + first 100 chars of last output + error class if any). Cap total enrichment time at ~10s (operator already waited 30 min; another 10s for diagnostic is fine). On any exception in the diagnostic path, swallow and emit the original `DISPATCH_TIMEOUT` clean — the diagnostic is best-effort and must not mask the timeout itself.
**Size**: S — ~40 LOC in `dispatch/__init__.py` + ~3 new unit tests (timeout-with-partial-progress, timeout-with-no-output, diagnostic-fails-silently-on-fetchOutput-error).
**Depends on**: nothing.
**Accept**: `DispatchPollTimeoutError` message body includes `Partial progress at timeout:` followed by per-cell status lines (`cell 1: pip rc=0`, `cell 2: FUSION_BICC_PASSWORD loaded`, `cell 3: in flight`, etc.); a fetchOutput failure during the diagnostic path is logged but doesn't mask the timeout; unit-tested by patching `fetch_output` to return a partial-executed notebook on the timeout path.
**Cross-ref**: `tests/live/TC29_rest_dispatch.md` Probe 4 (the manual `oci raw-request` walk this fix automates); `dispatch/__init__.py:dispatch_via_rest` poll_run try/except block.

### `[x]` P1.5ε-fix9 — Registry metadata split (`schema/registry_metadata.py` + `schema/plan_resolver.py`) — shipped 2026-06-03 on `oussama-dev-p1.5e-fix9` (947 unit tests green; live TC29 Probe 7 confirms dispatch `--dry-run` renders the "Would dispatch" + "Extra-plan prerequisites" Rich tables for the first time on the REST path)
**Why**: P1.5ε §Step 1c was the largest of the cross-boundary moves and landed deferred — only blocks dispatch-side dry-run **plan rendering** (the dispatcher currently returns `RunSummary.empty()` without populating `.plan` because there's no schema-level resolver that can build a plan without pulling `orchestrator/registry.py` + every `dimensions/*` / `transforms/*` callable into `sys.modules`). The dispatch dry-run still correctly validates preflight + skips the wheel/upload/dispatch; only the "Would dispatch" Rich table doesn't render with actual plan nodes. The original idea.md acceptance criterion was `aidp-fusion-bundle run --mode seed --env dev --dry-run` produces the would-run DAG print — currently it produces `Empty plan` for non-empty bundles when run via the REST path.
**When to revisit**: ship when operator workflow benefits from seeing the plan over REST before committing to a full dispatch (vs. the current `--inline --dry-run` path which works correctly today because it lives in-process and can import the registry freely).
**Fix sketch** (mechanical, no semantic changes — just extracting data from behavior in `orchestrator/registry.py`):
- New `schema/registry_metadata.py` with three frozen-dataclass metadata maps (`BRONZE_EXTRACT_METADATA`, `SILVER_DIM_METADATA`, `GOLD_MART_METADATA`) + three deferred-name maps (`KNOWN_DEFERRED_DATASETS`, `KNOWN_DEFERRED_DIMS`, `KNOWN_DEFERRED_MARTS`). Bronze entries projected from `BRONZE_EXTRACTS` (the "what's shipped" dict — NOT from `fusion_catalog.CATALOG` which contains entries like `hcm_worker_assignments` / `ap_aging_periods` that aren't runnable today; those land in `KNOWN_DEFERRED_DATASETS`). Deferred-name maps move verbatim from `orchestrator/registry.py:262`.
- New `schema/plan_resolver.py` with `resolve_dry_run_plan(bundle, mode, datasets, layers) → tuple[PlanNode, ...] + tuple[PrereqNode, ...]`. Moves `resolve_plan` from `orchestrator/__init__.py` but consumes only the neutral metadata (no callable references).
- Refactor `orchestrator/registry.py` to import metadata from the schema-level module; `SilverDimSpec` / `GoldMartSpec` gain a `.metadata` field referring back to the neutral object so `.depends_on_bronze` etc. read through a `@property` proxy.
- Refactor `commands/run.py:_render_summary` to drop the `_layer_for_spec` import fallback (PlanNode carries `.layer` directly).
- Wire `dispatch/__init__.py:dispatch_via_rest` dry-run path to call `resolve_dry_run_plan(...)` and populate `RunSummary.empty(plan=plan_nodes, prereqs=prereq_nodes)`.
Required tests: every behavior of today's `resolve_plan` reproduced under `schema/plan_resolver.py` — bronze filter happy path, filter typo `MissingDependencyError`, layer filter happy path + typo, disabled-dataset handling, deferred-bronze (`hcm_worker_assignments` + `ap_aging_periods`), deferred-dim, deferred-mart, undeclared-upstream rejection, extra-plan prereqs, empty bundle. Plus boundary tests: `import dispatch` still passes the `tests/unit/dispatch/test_imports.py` boundary check; `tests/unit/schema/test_plan_resolver.py` round-trips the moved cases.
**Size**: M — ~3-4h of careful refactoring + test relocation. Pure data-vs-behavior split; no semantic changes.
**Depends on**: nothing in plugin code; P1.5ε §Steps 1a + 1b + 1d already shipped (the cross-boundary moves this builds on top of).
**Accept**:
- `aidp-fusion-bundle run --mode seed --env dev --dry-run` (no `--inline`) renders the "Would dispatch" plan table + extra-plan prerequisites table — same output the `--inline --dry-run` path produces today.
- `tests/unit/dispatch/test_imports.py` still PASSes — the `schema/plan_resolver` + `schema/registry_metadata` imports don't pull `orchestrator/*`, `dimensions/*`, `transforms/*`.
- `tests/unit/schema/test_plan_resolver.py` covers every case `tests/unit/test_orchestrator_run.py::TestResolvePlan` covers today (mechanical relocation + boundary-test additions).
- 100% of pre-existing orchestrator unit tests stay green (no behavior change inside the engine).
**Cross-ref**: P1.5ε plan §Step 1c (the deferred work); §Step 8c (the dry-run consumer that needs it); `tests/unit/dispatch/test_imports.py` (the boundary lock that catches accidental engine imports).
