...
| `AIDPF-2013` | COA resolution | Active | A `semanticRole` chart-of-accounts mapping could not be resolved and the run is non-interactive with no accepted convention — fails closed. | Configure `profile.chartOfAccounts` (or the COA role mapping) for this tenant, or run interactively to accept a convention. |
