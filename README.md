...
| [`claude-to-codex-migrator`](claude-code-plugins/claude-to-codex-migrator/) | Convert Claude skills, plugins, commands, agents, and integration bundles into validated **Codex** Skills or plugins. Deterministic inventory → transform → cleanup-scan → validate pipeline with safety-hardened staging (ZIP limits, ownership-manifest output replacement, byte-level leftover scanning) and full decision/migration/cleanup/validation reports. Skills-only — no MCP server or external runtime. 38 unit tests passing. | [`craxelfn/claude-to-codex-migrator`](https://github.com/craxelfn/claude-to-codex-migrator) |
